...
class FinanceApp {
      ...
      if (user) {
        ...
        this.syncFromFirebase()
          .then(() => {
            ...
            // CORECCIÓN: Ocultar landing page y mostrar dashboard
          ...
          .catch((err) => {
            ...
            // CORRECCIÓN: Ocultar landing page y mostrar dashboard
            const landingSection = document.getElementById('landing');
            const dashboardSection = document.getElementById('dashboard');
            if (landingSection) {
              landingSection.classList.remove('active');
        ...
        // NUEVO: Mostrar SOLO landing page para anónimos (sin dashboard, sin stats, sin nada)
        const landingSection = document.getElementById('landing');
        const dashboardSection = document.getElementById('dashboard');
        if (landingSection) {
        ...
        console.log('👤 Usuario anónimo: Mostrando landing page');
      }
    });
  ...
  // ========================================
  // Ocultar/Mostrar botones que requieren autenticación
  // ========================================
  hideAuthRequiredButtons() {
    // Ocultar botón flotante de Fin
    const finFloatingBtn = document.getElementById('finFloatingBtn');
    if (finFloatingBtn) {
      finFloatingBtn.style.display = 'none';
    }

    // Ocultar botón FAB (botón + flotante)
    const fabBtn = document.getElementById('fabQuickExpense');
    if (fabBtn) {
      fabBtn.style.display = 'none';
    }

    console.log('✋ Botones Fin y + ocultados para usuario anónimo');
  }

  showAuthRequiredButtons() {
    // Mostrar botón flotante de Fin
    const finFloatingBtn = document.getElementById('finFloatingBtn');
    if (finFloatingBtn) {
      finFloatingBtn.style.display = 'flex';
    }

    // Mostrar botón FAB (botón + flotante)
    const fabBtn = document.getElementById('fabQuickExpense');
    if (fabBtn) {
      fabBtn.style.display = 'flex';
    }

    console.log('✅ Botones Fin y + mostrados para usuario autenticado');
  }
...
Escribe el número de la opción o cuéntame qué necesitas:`,
  ...
  openAuthModal(mode = 'login') {
    // NUEVO: Verificar si el usuario ha aceptado los términos primero
    if (window.termsManager && !window.termsManager.hasAcceptedTerms()) {
      console.log('⚠️ Usuario debe aceptar términos antes de login/registro');

      // Mostrar modal de términos
      window.termsManager.showTermsModal();

      // Esperar a que acepte los términos
      window.addEventListener('termsAccepted', () => {
        // Abrir modal de auth después de aceptar términos
