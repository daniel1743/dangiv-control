...
constructor() {
    ...
    this.expenses = savedData?.expenses || [
        {id: 1, description: "Supermercado", amount: 150, category: "Alimentación", necessity: "Muy Necesario", date: "2025-09-10", user: "Daniel", protected: false},
        {id: 2, description: "Gasolina", amount: 80, category: "Transporte", necessity: "Necesario", date: "2025-09-08", user: "Givonik", protected: false},
        {id: 3, description: "Netflix", amount: 12, category: "Entretenimiento", necessity: "Poco Necesario", date: "2025-09-05", user: "Daniel", protected: false},
        {id: 4, description: "Bombona de gas", amount: 45, category: "Servicios", necessity: "Muy Necesario", date: "2025-09-03", user: "Daniel", protected: false},
        {id: 5, description: "Cine", amount: 25, category: "Entretenimiento", necessity: "No Necesario", date: "2025-09-02", user: "Givonik", protected: false}
    ];

    this.goals = savedData?.goals || [
        {id: 1, name: "Vacaciones en la playa", target: 1500, current: 350, deadline: "2025-12-15"},
        {id: 2, name: "Fondo de emergencia", target: 1000, current: 750, deadline: "2025-11-30"},
        {id: 3, name: "Nueva laptop", target: 800, current: 200, deadline: "2025-10-31"}
    ];

    this.shoppingItems = savedData?.shoppingItems || [
        {id: 1, product: "Leche", quantity: 2, necessary: true, selected: false},
        {id: 2, product: "Pan", quantity: 1, necessary: true, selected: false},
        {id: 3, product: "Queso", quantity: 1, necessary: false, selected: false}
    ];
        ...
        if (delBtn) {
            delBtn.addEventListener('click', (e) => {
                ...
                if (typeof this.deleteExpense === 'function') {
                    ...
                    this.showToast('La función de eliminar se activará en el siguiente paso', 'info');
                }
            });
        }
    });
...
openSecurityModal(mode = 'change') {
  const modal = document.getElementById('securityModal');
  if (!modal) return this.showToast('Modal de seguridad no encontrado', 'error');
  ...
  this.modalMode = mode;
  const newPassSection = document.getElementById('newPassSection');
  if (newPassSection) newPassSection.style.display = (mode === 'change') ? '' : 'none';

  // ajustar texto del botón
  const saveBtn = document.getElementById('modalSavePasswordsBtn');
  if (saveBtn) saveBtn.textContent = (mode === 'change') ? 'Guardar' : 'Confirmar';
...
savePasswordsFromModal() {
  ...
  if (!modal || !curDanielEl || !curGivonikEl || !newDanielEl || !confirmDanielEl || !newGivonikEl || !confirmGivonikEl) {
  ...
  this.showToast('Contraseñas actualizadas con doble autorización', 'success');
    ...
    renderNecessityChart() {
        ...
        const necessaryExpenses = this.expenses.filter(exp => 
            exp.necessity === 'Muy Necesario' || exp.necessity === 'Necesario'
        ).reduce((sum, exp) => sum + exp.amount, 0);

        const unnecessaryExpenses = this.expenses.filter(exp => 
            exp.necessity === 'Poco Necesario' || exp.necessity === 'No Necesario' || exp.necessity === 'Compra por Impulso'
        ).reduce((sum, exp) => sum + exp.amount, 0);
    ...
    renderUnnecessaryExpenses() {
        ...
        const unnecessary = this.expenses.filter(exp => 
            exp.necessity === 'No Necesario' || exp.necessity === 'Compra por Impulso'
        );
    ...
    renderShoppingList() {
        ...
        this.shoppingItems.forEach((item, index) => {
            ...
            itemEl.innerHTML = `
                <input type="checkbox" class="shopping-checkbox" ${item.selected ? 'checked' : ''} 
                       data-index="${index}">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-content">
                        ...
                        <p class="stat-value" style="color: ${available >= 0 ? 'var(--color-success)' : 'var(--color-error)'}">
                    ...
                    <div class="recommendation-item">
                        ...
                        <div class="recommendation-text">
                            ${available >= 0 ? 
                                ...
                                `Estás gastando $${Math.abs(available)} más de tus ingresos. Revisa tus gastos innecesarios.`
                            }
                        </div>
                    </div>
                    ${this.expenses.filter(exp => exp.category === 'Servicios').length > 0 ? `
                        <div class="recommendation-item">
                            <i class="fas fa-question-circle recommendation-icon"></i>
                            <div class="recommendation-text">
                                Este mes compraste ${this.expenses.filter(exp => exp.description.toLowerCase().includes('bombona')).length} bombona(s) de gas. ¿Era necesario?
                            </div>
                        </div>
                    ` : ''}
                </div>
